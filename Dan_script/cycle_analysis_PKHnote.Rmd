...
# Extract the info we want from the huge table: Genotype, Treatment, Sampling.Day
## First, combine all the things we want
...
## Make a new dataframe and divide the combined info back
...
## The order then is used to set the levels of the ExpIDs (why?)
Expts = Expt_design$ExpID
sample_info$ExpID = factor(sample_info$ExpID,levels = Expts)

# Sort the sample (I think the data were pre-sorted, but Dan do it again I guess just in case.)
sample_info = sample_info[with(sample_info,order(Treatment,Sampling.Day,Genotype)),]
## Use the sorted order to arrange the columns in exp dataframe
...
# I don't understand how this work because it is not a classical tapply! But this help you sum up the transcripts according to their gene names, anyway... 
...
# I don't have the gene list file
...
JTK_fun = function(data,sample_info){
  ...
  for(expt in Expts) {
    ...
    # We can input unbalance data? Yes! Check the jdkdist function
